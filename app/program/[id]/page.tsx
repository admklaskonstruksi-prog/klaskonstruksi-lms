...
export default async function PublicCourseDetail({ params }: { params: { id: string } }) {
  ...
  if (error || !course) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-gray-500 mb-8 max-w-sm">Maaf, kelas yang Anda cari mungkin sudah dihapus atau URL salah.</p>
        <Link href="/program" className="bg-[#F97316] text-white px-8 py-3.5 rounded-xl font-bold">Kembali ke Katalog</Link>
      </div>
    );
  }

  // 2. CEK AMAN KATEGORI (Mencegah error jika Kategori bernilai null)
  let subCat = null;
  let levelCat = null;
  ...
  // UBAH HARGA MENJADI ANGKA (Mencegah crash toLocaleString)
         ...
         <div className="max-w-7xl mx-auto relative z-10 grid grid-cols-1 lg:grid-cols-3 gap-12">
            <div className="lg:col-span-2">
               ...
               <div className="flex items-center gap-3 mb-5">
                 <span className="bg-[#00C9A7]/20 text-[#00C9A7] px-4 py-1.5 rounded-full text-xs font-bold border border-[#00C9A7]/30 uppercase">
               ...
               <h1 className="text-4xl md:text-6xl font-black mb-6 leading-tight text-white">{course.title}</h1>
               <p className="text-gray-300 text-lg md:text-xl mb-10 max-w-3xl leading-relaxed">{course.description}</p>
               ...
               <div className="space-y-8">
                  ...
                  <div className="space-y-5">
                     ...
                     {chapters?.map((chapter: any, idx: number) => (
                        <div key={chapter.id} className="bg-white border border-gray-100 rounded-3xl shadow-sm overflow-hidden transition-all">
                           <div className="p-6 bg-gray-50 font-extrabold text-gray-950 flex flex-col md:flex-row md:justify-between md:items-center border-b border-gray-100 gap-2">
                              ...
                              <span className="text-xs font-bold text-gray-500 bg-white px-3 py-1.5 rounded-lg border border-gray-100 w-max">{chapter.lessons?.length || 0} Materi</span>
                           </div>
                           <div className="p-3 space-y-1">
                              {chapter.lessons?.sort((a:any, b:any) => a.position - b.position).map((lesson: any, lIdx: number) => (
                                 <div key={lesson.id} className="flex items-center gap-4 p-4 hover:bg-gray-50 rounded-2xl transition group">
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition ${lesson.is_preview ? 'bg-[#00C9A7]/10 text-[#00C9A7]' : 'bg-gray-100 text-gray-400'}`}>
                                       <PlayCircle size={20} />
                                    </div>
                                    <p className={`text-sm flex-1 ${lesson.is_preview ? "font-bold text-gray-900" : "text-gray-700 font-medium"}`}>{lIdx + 1}. {lesson.title}</p>
                                    {lesson.is_preview && <span className="ml-auto text-[10px] bg-teal-50 text-[#00C9A7] font-black px-3 py-1.5 rounded-full uppercase tracking-wider">Preview</span>}
                                 </div>
                              ))}
                           </div>
                        </div>
            ...
            <div className="lg:col-span-1 lg:sticky lg:top-28 lg:self-start -mt-20 lg:-mt-64 relative z-30">
               <div className="bg-white text-gray-950 rounded-3xl p-7 shadow-2xl shadow-gray-200/70 border border-gray-100">
                  ...
                  <div className="border-t border-gray-100 pt-7 mt-7 space-y-4">
                     {isOwned ? (
                       <Link href={`/dashboard/learning-path/${course.id}`} className="block w-full bg-gray-950 text-white text-center py-4 rounded-2xl font-black text-lg hover:bg-gray-800 transition shadow-lg">
                          Masuk Ruang Belajar
                       </Link>
                     ) : (
